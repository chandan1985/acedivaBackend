...
userController.post("/send-otp", async (req, res) => {
    try {
        ...
        const appHash = "ems/3nG2V1H"; // App Hash
        ...
        let optResponse = await axios.post(
            `https://api.authkey.io/request?authkey=${process.env.AUTHKEY_API_KEY
            }&mobile=${phoneNumber}&country_code=91&sid=${process.env.AUTHKEY_SENDER_ID
...
userController.post("/create-admin", async (req, res) => {
    try {
        ...
        let admin = await User.create({ ...req.body, role: "admin" });
